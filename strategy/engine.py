...
from core.ai_market_brain import MarketIntelligenceBrain
from core.ai_memory import record_market
...
class StrategyEngine:
    ...
    def __init__(self):
        self.ai_brain = MarketIntelligenceBrain()
    ...
    def analyze(self, symbol: str, side: str, df, orderbook=None) -> Dict[str, Any]:
        ...
        try:
            trade_intelligence = E.analyze_setup(df, side, price, atr, symbol) if getattr(E, "TRADE_INTELLIGENCE_AVAILABLE", False) else {}
        except Exception:
            trade_intelligence = {}
        ai_market = self.ai_brain.evaluate(
            symbol, side, df, orderbook,
            legacy_analysis=legacy_view,
            trade_intelligence=trade_intelligence,
        )
        # AI is an evidence layer.  Its score is reported independently and is
        # not silently added to the legacy strategy score.  The downstream queue
        # can opt into bounded AI gating through AI_MARKET_MODE.
        try:
            record_market(ai_market)
        except Exception:
            pass
        return {
            "symbol": symbol,
