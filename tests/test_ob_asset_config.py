...
def _load_engine():
    ...
    if orig_engine is not None:
        spec = importlib.util.spec_from_file_location("_ob_asset_fresh_engine", orig_engine.__file__)
        engine = importlib.util.module_from_spec(spec)
        sys.modules["_ob_asset_fresh_engine"] = engine
        try:
            spec.loader.exec_module(engine)
        finally:
            sys.modules.pop("_ob_asset_fresh_engine", None)
    else:
