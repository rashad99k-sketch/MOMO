...
class ExchangeSyncService:
    ...
    def reconcile(self, symbol, local_state):
        ...
        with _TRADE_LOCK:
            ...
            _track_roe_extremes(snap.roe_pct)
...
class LiveTradeManager:
    ...
    def _apply_dynamic_profit_and_exit_rules(self, *, symbol, mark_price, atr, side,
        ...
        if not STATE.get("dynamic_reversal_exit_done", False):
            ...
            combined_medium = (ifvg_strong or (ifvg_weak and opposed_mom)) and \
                              ...
                              roe > 0
            if (reversal_strong or combined_strong) and health_score < 50 and confidence >= 0.7:
                ...
                # Medium evidence: de-risk 50% and convert remainder to a runner
                # with a breakeven guard, rather than a full blind close.
                STATE["dynamic_reversal_exit_done"] = True
                close_partial(0.5)
                STATE["tp1_hit"] = True
                STATE["runner_mode"] = True
                ...
                log_execution(
                    ...
                    f"partial 50% + runner | {symbol} ROE={roe:.2f}% health={health_score:.1f} "
        ...
        if not STATE.get("dynamic_partial_done", False) and not STATE.get("tp1_hit", False):
            ...
            if (early_reversal_profit or is_correction) and roe >= roe_target and cont < 0.62:
                ...
                STATE["synthetic_sl"] = entry
                log_execution(
                    f"[DYNAMIC] Early profit bank ({trade_type_cur}/{asset_class}"
                    f"{'/CORRECTION' if is_correction else ''}) "
                    f"roe={roe:.2f}% >= {roe_target:.2f}% -> partial 50% + runner",
                    "SUCCESS"
                )
    ...
    def manage_live_trade(self):
        ...
        if self.lifecycle_state == TradeLifecycleState.OPEN_PENDING_CONFIRMATION:
            self._set_lifecycle(TradeLifecycleState.LIVE)
        now = time.time()
    ...
    def _apply_management(self, symbol, now):
        ...
        if not STATE.get("tp1_hit", False) and tp1_reached:
            ...
            if tp1_hold_score >= 8:
                ...
                # execute_entry) over the legacy fixed 4% reference.
                tp1_price = STATE.get("dynamic_tp1", entry * (1 + 0.04) if side == "BUY" else entry * (1 - 0.04))
...
try:
    from core.ai_memory import record_trade_event as _record_ai_trade_event
except Exception:
    _record_ai_trade_event = None
...
def _track_roe_extremes(roe):
    ...
    if not STATE.get("open"):
        return
    STATE["peak_roe"] = max(float(STATE.get("peak_roe", 0.0) or 0.0), value)
    STATE["min_roe"] = min(float(STATE.get("min_roe", 0.0) or 0.0), value)
...
def _journal_trade_event(event, *, symbol="", side="", reason="", detail="",
    ...
    if event == _tj.TRADE_CLOSED:
        try:
            if callable(_record_ai_trade_event):
                _record_ai_trade_event("TRADE_OUTCOME", {
                    "trade_id": tid, "symbol": sym, "side": side_v,
                    "result": (st.get("last_trade_summary") or {}).get("result"),
                    "close_reason": st.get("close_reason") or reason,
                    "realized_pnl_usdt": st.get("realized_pnl_usdt", 0.0),
                    "realized_pnl_pct": st.get("realized_pnl_pct", 0.0),
                    "mfe_roe": st.get("peak_roe", 0.0),
                    "mae_roe": st.get("min_roe", 0.0),
                    "duration_sec": ((time.time() - float(st.get("entry_time"))) if st.get("entry_time") else None),
                    "ai_market": st.get("ai_market", {}) or {},
                    "trade_intelligence": st.get("trade_intelligence", {}) or {},
                    "thesis": st.get("trade_thesis", {}) or {},
                })
        except Exception:
            pass
    try:
...
def open_position(side, amount, symbol, client_order_id=None):
    ...
    try:
        ...
        order, cid = _order_manager.submit_order(symbol, side, amount, LEVERAGE, client_order_id)
        if order is None:
...
def _record_ai_trade_open(symbol, side, price, score, reason):
    """Persist the exact AI/strategy evidence attached to an opened trade."""
    try:
        if callable(_record_ai_trade_event):
            _record_ai_trade_event("TRADE_ENTRY_AI_RECEIPT", {
                "trade_id": STATE.get("trade_id"),
                "symbol": symbol,
                "side": side,
                "entry_price": float(price or 0),
                "strategy_score": float(score or 0),
                "entry_reason": str(reason or "")[:500],
                "entry_ts": float(STATE.get("entry_time") or time.time()),
                "ai_market": STATE.get("ai_market", {}) or {},
                "trade_intelligence": STATE.get("trade_intelligence", {}) or {},
...
def execute_entry(side, symbol, price, sl, tp1, tp2, score, reason, atr_val, trade_type, entry_type, classification):
    ...
    # AI evidence is optional and observational by default. The portfolio layer
    # places the exact candidate snapshot into STATE before this authority runs.
    STATE.setdefault("ai_market", {})
    ...
    # ---- Phase-3 (G1): anti-scalp ATR TP floor ----
    # Persisted separately (dynamic_tp1/dynamic_tp2) so legacy STATE values stay
    # intact; the management layer prefers these targets (TP1_DELAY reads
    # dynamic_tp1, the TP2 percent is widened by tp2_atr).
    dyn_tp1, dyn_tp2 = apply_atr_tp_floor(
    ...
    if PAPER_MODE:
        ...
        _record_ai_trade_open(symbol, side, price, score, reason)
        return True
    ...
    if order:
        ...
        _record_ai_trade_open(symbol, side, STATE.get("entry", price), score, reason)
        return True
    else:
...
def promote_to_queue():
    ...
    for sym, entry, zentry in candidates:
        ...
        cand.ai_market = entry.get("ai_market", {}) or {}
...
class ExecutionCandidate:
    ...
    ai_market: dict = field(default_factory=dict)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'ai_market': self.ai_market,
            'ai_score': float((self.ai_market or {}).get('score', 0.0) or 0.0),
            'ai_confidence': float((self.ai_market or {}).get('confidence', 0.0) or 0.0),
            'ai_action': (self.ai_market or {}).get('action', 'WAIT'),
            'ai_zone': (self.ai_market or {}).get('preferred_zone', {}),
        }
...
class ExecutionQueue:
    ...
    def _select_strong_ob(self, df, side, atr, cfg=None):
        ...
        ob_score = min(100.0, self._causal_ob_score(best_disp, vol_ratio, ztouch, freshness) + synergy["bonus"])
